defmodule RecipebookWeb.Schema.Mutations.RecipeTest do
  ...
  describe "@createRecipe" do
    ...
    test "able to create recipe", context do
      ...
      assert {:ok, %{data: %{"createRecipe" => result}}} =
               Absinthe.run(@create_recipe_doc, Schema,
                 variables: raw_recipe,
                 context: %{current_user: user}
               )

      assert {:ok, found_recipe} = Cookbook.find_recipe(%{name: result["name"]})
      assert found_recipe.categories === raw_recipe["categories"]
    end
  ...
  describe "@updateRecipe" do
    ...
    test "able to update recipe", context do
      ...
      assert {:ok, %{data: %{"updateRecipe" => result}}} =
               Absinthe.run(@update_recipe_doc, Schema,
                 variables: new_recipe,
                 context: %{current_user: user}
               )

      assert {:ok, found_recipe} = Cookbook.find_recipe(%{id: old_recipe_id})
      assert found_recipe.name === new_recipe["name"]
      assert found_recipe.name === result["name"]
