defmodule RecipebookWeb.Schema.Subscriptions.RecipeTest do
  ...
  describe "@recipe_creation_subscription" do
    test "sends an recipe when one of the user ids create the recipe", %{
      ...
      ref =
        push_doc(socket, @recipe_create_subs_doc, variables: %{"user_ids" => [user.id, user2.id]})

      assert_reply ref, :ok, %{subscriptionId: subscription_id}

      assert {:ok, _} =
               Absinthe.run(@create_recipe_doc, Schema,
                 variables: recipe_raw_data,
                 context: %{current_user: user, pubsub: RecipebookWeb.Endpoint}
               )
