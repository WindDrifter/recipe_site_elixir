defmodule RecipebookWeb.Schema.Queries.RecipeTest do
  ...
  describe "@recipes" do
    ...
    test "able to get recipes via ingredients", context do
      ...
      assert {:ok, %{data: data}} = Absinthe.run(@all_recipes_doc, Schema,
      variables: %{
  ...
  describe "@recipe" do
    ...
    test "able to get recipe via id", context do
      ...
      assert {:ok, %{data: data}} = Absinthe.run(@get_recipe_doc, Schema,
      variables: %{
        "id" => recipe.id
        }
      )
      assert data["recipe"]["name"] === recipe.name
      assert Enum.count(data["recipe"]["recipe_ingredients"]) === Enum.count(recipe.recipe_ingredients)
      assert List.first(data["recipe"]["recipe_ingredients"])["name"]

    end
    test "return error when id doesn't exist", _context do
      assert {:ok, errors} = Absinthe.run(@get_recipe_doc, Schema,
      variables: %{
        "id" => "333333333"
        }
      )
