defmodule RecipebookWeb.Schema.Queries.StatTest do
  ...
  describe "@stat" do
    test "Grab stat by category name" do
      ...
      assert {:ok, %{data: data}} =
               Absinthe.run(@get_stats_doc, Schema,
  ...
  describe "@categories_stats" do
    test "Grab stat by category name" do
      ...
      assert {:ok, %{data: data}} =
               Absinthe.run(@get_category_stats_doc, Schema,
      ...
      assert Enum.empty?(
               Enum.reject(categories_stats, fn %{"name" => name} ->
                 Enum.member?(targeted_categories, name)
               end)
             )
    end
