defmodule Recipebook.Account do
  ...
  @cannot_find_user_tuple {:error, ErrorMessage.not_found("Cannot find user")}
  ...
  def follow_user(user, %{id: id} = _params) do
    with {:ok, following_user} <- find_user(%{id: id}),
         {:ok, _user} <-
           Actions.update(User, user,
             following: [%{id: following_user.id} | turn_following_list_into_list_of_ids(user)]
           ) do
    ...
    case Authentication.verify_user(%{username: user.username, password: current_password}) do
      {:ok, _user} ->
        if user.id === id do
          ...
          {:error, ErrorMessage.unauthorized("You do not have permission to change user details")}
        end
  ...
  @spec create_user(map()) :: {:ok, any()} | {:error, ErrorMessage.t()}
  ...
  def save_recipe(user, %{recipe_id: recipe_id} = _params) do
    with {:ok, recipe} <- Actions.find(Recipe.setup_query(), %{id: recipe_id}),
         {:ok, _user} <-
           Actions.update(User, user,
             saved_recipes: [%{id: recipe.id} | get_list_of_save_recipe_ids(user)]
           ) do
  ...
  def unsave_recipe(user, %{recipe_id: recipe_id} = _params) do
    case Actions.update(User, user, saved_recipes: get_list_of_save_recipe_ids(user, recipe_id)) do
      {:ok, result} -> {:ok, %{message: "Successfully unsave recipe"}}
      {:error, message} -> {:error, message}
    end
  end
